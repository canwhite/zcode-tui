...
function metricViews(): ViewOptions[] {
  return [
    histogramView("qcode.agent.turn.duration", [1, 5, 10, 30, 60, 300, 600]),
    histogramView("qcode.model.*.duration", [0.25, 1, 2, 5, 10, 30, 60, 120]),
    histogramView(
      "qcode.context.compaction.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView(
      "qcode.detached.operation.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("qcode.agent.step.duration", [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600]),
    histogramView("qcode.tool.execution.duration", [0.01, 0.1, 0.5, 1, 5, 30, 120, 300]),
    histogramView(
      "qcode.command.execution.duration",
      [0.01, 0.05, 0.1, 0.5, 1, 5, 30, 120, 300, 600],
    ),
    histogramView("qcode.*.time_to_first_*", [0.25, 0.5, 1, 2, 5, 10, 30, 60]),
    histogramView("qcode.model.call.attempts", [1, 2, 3, 5, 8]),
    cardinalityView("qcode.model.attempt.tokens"),
    cardinalityView("qcode.model.attempt.stream_stall.count"),
    cardinalityView("qcode.telemetry.creation_drop.count"),
    cardinalityView("qcode.telemetry.abandoned.count"),
  ];
}
...
function telemetryResourceAttributes(
  ...
  return compactResourceAttributes({
    ...
    "qcode.build.commit_id": resource.buildCommitId,
    "qcode.device.installation_id": options.includeInstallationId
      ? resource.installationId
      : undefined,
    "qcode.product.version": resource.productVersion,
    "qcode.runtime.distribution": resource.runtimeDistribution,
    "qcode.runtime.surface": resource.runtimeSurface,
    "qcode.telemetry.schema_owner": "cli",
    "qcode.telemetry.schema_version": TELEMETRY_SCHEMA_VERSION,
  });
}
