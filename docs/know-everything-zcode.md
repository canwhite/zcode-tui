...
| 发布              | release-it + esbuild SEA（Node 单可执行）                          | `apps/zcode-cli` 有 SEA 打包链；native-search 二进制按平台预置并校验 sha256                                     |
...
flowchart TD
  ...
  DESKTOP --> SERVER
  DESKTOP --> SERVICES
  CLIAPP["apps/zcode-cli<br/>Agent 运行时（独立构建产物）"]
...
| `packages/services/src/zcode-agent/zcodeAgentProcessManager.ts:356,379` | 以**路径字符串**硬编码 Runtime 产物：`findUpward("apps/zcode-cli/packages/cli/dist/zcode.cjs")` 与 `.../src/main.ts`，并特判 bytecode 产物名。无编译期依赖（零 `import` 自 `apps/zcode-cli`），但布局一变即静默失败                                                                                                                                                                                                                         | **Medium** | 4.2 未校验假设 | `/planning`             |
| `packages/services/src/skills/skillsService.ts:61,137`                  | 注释自承"对齐" `apps/zcode-cli/packages/adapters/src/skills/index.ts:19` 与 `roots.ts:60-72` —— 同一份逻辑在两侧各实现一遍，靠注释维持同步                                                                                                                                                                                                                                                                                                  | **Medium** | 4.2 双重维护   | `/planning`             |
...
| 位置                                                               | 问题                                                                                                                                                                                                                                   | 严重度   | 发现路径                    | Next Skill  |
| ------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | --------------------------- | ----------- |
| `apps/zcode-cli/.husky/pre-commit`                                 | Hook 内容为 `pnpm lint` + **`pnpm test`**，但仓库**任何 `package.json` 都没有 `test` script**（根与 `apps/zcode-cli` 均已核实）。即 hook 必然失败；且 `git config core.hooksPath` 为空、根目录无 `.husky/`，该 hook 当前**完全未接线** | **High** | 4.3 异常吞噬 / 4.5 无测试面 | `/diagnose` |
...
| `apps/zcode-cli/packages/core/src/runtime/methods/turn-loop.ts:43` | `runRegularTurnLoop` 的 `while(true)` 是承重循环，终止条件分散在 `turnMachine` / `turnControl.stopTurnAfterResult` / `compact` 多处；无状态机层面的显式终止不变量文档。**未发现 actual bug，属风险面**                                 | **Low**  | 4.3 无限循环风险            | `/diagnose` |
