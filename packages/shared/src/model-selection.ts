...
export const QCODE_MODEL_REASONING_SEPARATOR = "$";
...
export function formatModelPickerValue(selection: ModelSelection | undefined): string {
  ...
  return reasoningLevel ? `${base}${QCODE_MODEL_REASONING_SEPARATOR}${reasoningLevel}` : base;
}
...
export function parseModelPickerValue(value: string): ModelSelection {
  ...
  const reasoningSeparatorIndex = rawModelId.indexOf(QCODE_MODEL_REASONING_SEPARATOR);
