...
export const QCODE_OFFICIAL_PLUGIN_MARKETPLACE = "qcode-plugins-official";
export const QCODE_INLINE_PLUGIN_MARKETPLACE = "inline";
export const QCODE_PLUGIN_HOST_COMMAND = "__zcode-plugin-host";
/**
 * 隐藏子命令：dynamic workflow 的沙箱子进程入口（`__zcode-dwf-child <entry path>`；argv 末位是
 * harness 写好的入口文件路径，payload 不过命令行）。
 *
 * 与 {@link QCODE_PLUGIN_HOST_COMMAND} 同族、同机制：SEA 单文件二进制不解释 Node CLI 旗标，
 ...
 */
export const QCODE_DWF_CHILD_COMMAND = "__zcode-dwf-child";

export function isOfficialMarketplaceId(id: string): boolean {
  return id === QCODE_OFFICIAL_PLUGIN_MARKETPLACE;
}
