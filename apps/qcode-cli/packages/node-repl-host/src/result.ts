...
} from "@qcode/contracts/mcp";
import { isOfficialCuaImageRefText } from "@qcode/zcode-cua/frame-contract";
import { CUA_APP_ASSOCIATIONS_META_KEY } from "@qcode/zcode-cua/host-display-contract";
import type { NodeReplRunResult } from "@qcode/core/repl";
...
export function toMcpRunResult(run: NodeReplRunResult): CallToolResult {
  ...
  delete responseMeta[QCODE_MCP_BROWSER_SCREENSHOT_CONTENT_INDICES_META_KEY];
  ...
  delete responseMeta[QCODE_MCP_NODE_REPL_CUA_APP_META_KEY];
  if (run.cuaApp) responseMeta[QCODE_MCP_NODE_REPL_CUA_APP_META_KEY] = run.cuaApp;
      ...
      ? {
          _meta: {
            ...
            ...((run.images?.length ?? 0) > 0 || structuredContent.some((block) => block.type === "image")
              ? { "qcode/nodeReplEmittedImage": true }
              : {}),
