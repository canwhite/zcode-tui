...
export function resolveIntranetDepsBaseUrl(env: IntranetEnv = readProcessEnv()) {
  const depsBaseUrl = env.QCODE_DEPS_BASE_URL?.trim();
  if (depsBaseUrl) {
  ...
  if (!host)
    throw new Error(
      "Configure QCODE_DEPS_BASE_URL or INTRANET_MACHINE_HOST in .env before downloading internal dependencies",
    );
