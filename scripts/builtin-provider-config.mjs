...
export async function loadBuiltinProviderConfig({ root = repositoryRoot, env = process.env } = {}) {
  ...
  const sourcePath = resolve(
    root,
    env.QCODE_BUILTIN_PROVIDER_CONFIG_FILE?.trim() || "config/provider/qcode-builtin.json",
  );
  try {
    ...
    const { decodeZCodeBuiltinRelease } = await tsImport(
      pathToFileURL(resolve(repositoryRoot, "packages/provider-node/src/qcode-builtin-release.ts"))
        .href,
      import.meta.url,
    );
...
export async function stageBuiltinProviderConfig({ directory, ...options }) {
  ...
  await writeFile(resolve(directory, "qcode-builtin.json"), config.content, "utf8");
  return config;
}
