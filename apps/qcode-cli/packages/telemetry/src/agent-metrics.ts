...
export class OtelAgentTelemetryMetrics implements AgentTelemetryMetricRecorder {
  ...
  constructor(meter: Meter) {
    this.durations = {
      agent_turn: duration(meter, "qcode.agent.turn.duration"),
      agent_step: duration(meter, "qcode.agent.step.duration"),
      tool_execution: duration(meter, "qcode.tool.execution.duration"),
      command_execution: duration(meter, "qcode.command.execution.duration"),
      context_compaction: duration(meter, "qcode.context.compaction.duration"),
      detached_operation: duration(meter, "qcode.detached.operation.duration"),
      model_call: duration(meter, "qcode.model.call.duration"),
      model_attempt: duration(meter, "qcode.model.attempt.duration"),
    };
    this.modelCallAttempts = meter.createHistogram("qcode.model.call.attempts", {
      description: "Physical provider attempts per logical model call",
      unit: "{attempt}",
    });
    this.modelTokens = meter.createCounter("qcode.model.attempt.tokens", {
      description: "Observed provider token delta by token type",
      unit: "{token}",
    });
    this.modelAttemptFirstProviderEvent = duration(
      meter,
      "qcode.model.attempt.time_to_first_provider_event",
    );
    this.modelAttemptFirstContent = duration(meter, "qcode.model.attempt.time_to_first_content");
    this.modelAttemptFirstText = duration(meter, "qcode.model.attempt.time_to_first_text");
    this.modelAttemptStreamStalls = meter.createCounter("qcode.model.attempt.stream_stall.count", {
      description: "Observed stream stalls",
      unit: "{stall}",
    });
    this.modelAttemptStreamMaxIdle = duration(meter, "qcode.model.attempt.stream_max_idle");
    this.commandFirstOutput = duration(meter, "qcode.command.execution.time_to_first_output");
    this.creationDrop = meter.createCounter("qcode.telemetry.creation_drop.count", {
      description: "Spans rejected before creation by runtime capacity protection",
      unit: "{span}",
    });
    this.abandoned = meter.createCounter("qcode.telemetry.abandoned.count", {
