...
import { QCODE_PLUGIN_HOST_COMMAND } from "@qcode/contracts/plugins";
import {
...
import { QCODE_CUA_OFFICIAL_PLUGIN_ID, QCODE_PLUGIN_ID_ENV_KEY } from "@qcode/shared/mcp";
import type { RunContext } from "@qcode/shared-types";

const HOST_USAGE = `${QCODE_PLUGIN_HOST_COMMAND} <server-path> [-- <server-arg>...]`;
...
export function isPluginHostInvocation(argv: readonly string[]): boolean {
  return argv[0] === QCODE_PLUGIN_HOST_COMMAND;
}
...
export async function runPluginHostCommand(ctx: RunContext, argv: string[]): Promise<number> {
  ...
  try {
    ...
    const originalBrokerSocket = process.env[QCODE_CUA_BROKER_SOCKET_ENV_KEY];
    // shared node_repl 把同一凭据组恢复到环境，由 broker bridge 读取；旧的独立 CUA
    // MCP 不再拥有执行入口。
    process.argv = [process.execPath, serverPath, ...serverArgs];
    if (capturedBrokerCredentials.socket && process.env[QCODE_CUA_NODE_REPL_HOST_ENV_KEY] === "1") {
      process.env[QCODE_CUA_BROKER_SOCKET_ENV_KEY] = capturedBrokerCredentials.socket;
    }
    try {
      await module.main();
    } finally {
      ...
      if (originalBrokerSocket === undefined) {
        delete process.env[QCODE_CUA_BROKER_SOCKET_ENV_KEY];
      } else {
        process.env[QCODE_CUA_BROKER_SOCKET_ENV_KEY] = originalBrokerSocket;
      }
    }
...
function assertCapturedBrokerLaunchIsAuthorized(credentials: CapturedBrokerCredentials): void {
  ...
  const pluginId = process.env[QCODE_PLUGIN_ID_ENV_KEY]?.trim().toLowerCase();
  ...
  // 残留读取方只能靠 CLI 自己的 typecheck 发现（根 `pnpm typecheck` 不含 apps/qcode-cli）。
  ...
  if (
    ...
    pluginId !== QCODE_CUA_OFFICIAL_PLUGIN_ID ||
    process.env[QCODE_CUA_NODE_REPL_HOST_ENV_KEY] !== "1"
  ) {
    throw new Error(
