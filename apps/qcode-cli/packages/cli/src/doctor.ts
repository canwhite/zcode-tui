    ...
    if (!floor) {
      ...
      checks.push({
        ...
        ...(meets
          ? {}
          : {
              fix: `切换到满足 ${expectedNode} 的 Node（仓库 pin 见 apps/qcode-cli/.node-version）后重试`,
            }),
      });
    }
  }
...
function checkCommandReachable(env: CliEnv): DoctorCheck {
  const resolved = firstOnPath("qcode", env.PATH);
  if (!resolved) {
    return {
      id: "install.command",
      label: "qcode 命令可达",
  ...
  return { id: "install.command", label: "qcode 命令可达", status: "pass", detail: resolved };
}
