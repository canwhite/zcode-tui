...
> **执行后状态（判据修正后）**：Node 运行时已改为**不本地化**（属第三方通用包），因此"下载错版本"这一具体形态不再存在——仓库里不再存任何 Node 归档。但**本风险的根因未被消除，只是转移了**：`build-sea.mjs:301` 仍用 `process.versions.node`，仍会在本地无归档时**静默回源 `nodejs.org`**。所以第 3 条缓解（把静默改为指名失败）**仍然必须做**——否则"不依赖远程资源"在 `build:sea` 这条路径上依然是句空话，且失败会推迟到断网现场才暴露。台账中 node-\* 条目保留登记（标 `prerequisite` / `vendoring: excluded`）正是为了让这个获取点在对账中始终可见。
