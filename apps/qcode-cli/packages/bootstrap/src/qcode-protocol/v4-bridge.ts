...
// - 命令执行 = V4CommandExecutor（qcode-protocol-v4/commands/，原生直驱 core）；
...
import { V4CommandExecutor } from "../qcode-protocol-v4/commands/executor.js";
import { V4QueuePromotionLeaseUnavailableError } from "../qcode-protocol-v4/commands/handlers/queue.js";
import { V4CapabilityUnsupportedError } from "../qcode-protocol-v4/commands/handlers/interaction-background.js";
import {
  buildColdFileChangeSummaries,
  readConversationFileChangesFromEvents,
} from "../qcode-protocol-v4/cold-file-change-summaries.js";
import {
  loadPersistedConversationMaterialization,
  mergeColdConversationEvents,
} from "../qcode-protocol-v4/cold-event-merge.js";
import { lookupGlobalCreateSessionCommand } from "../qcode-protocol-v4/create-session-command-fact.js";
import type { V4CommandCoreHost } from "../qcode-protocol-v4/commands/types.js";
import type {
  ConversationRowTargetResolution,
  SessionUsageSeed,
} from "../qcode-protocol-v4/product-projection.js";
import { PersistentCommandIndex } from "../qcode-protocol-v4/persistent-command-index.js";
import { queueItemIdForCommand } from "../qcode-protocol-v4/command-inbox.js";
import { resolveStableForkTargetFromTranscript } from "../qcode-protocol-v4/stable-fork-target.js";
import { shouldAutoDrainV4QueueHead } from "../qcode-protocol-v4/queue-auto-drain.js";
import { persistAssistantFeedback } from "../qcode-protocol-v4/assistant-feedback-persistence.js";
import {
  TASK_LIST_SESSION_TYPES,
  isTaskListSessionType,
} from "../qcode-protocol-v4/task-list-session-membership.js";
import {
  loadPersistentCommandFacts,
  savePersistentCommandFact,
} from "../qcode-protocol-v4/persistent-command-facts.js";
import {
  ConversationV4Gateway,
  V4CommandNotImplementedError,
} from "../qcode-protocol-v4/v4-gateway.js";
import {
...
import { HYDRATION_TRACE_ID } from "../qcode-protocol-v4/projection-state.js";
...
async function replayDynamicWorkflowRunEvents(
  ...
  try {
    ...
    context.logger?.warn("v4 hydrate dynamic workflow replay failed", {
      ...
      event: "qcode_protocol.v4.hydrate_workflow_replay_failed",
  ...
  const loadStoredSessionSummaries = async (
    ...
    try {
        ...
        try {
          ...
          if (claimedCount > 0) {
            context.logger?.info("legacy remote sessions claimed by task-index allowlist", {
              claimedCount,
              event: "qcode_protocol.v4.sessions_index_legacy_remote_claimed",
              module: "bootstrap.zcode_protocol",
              workspaceId,
            });
          }
          ...
          context.logger?.warn("legacy remote sessions claim failed; continuing strict load", {
            error: error instanceof Error ? error.message : String(error),
            event: "qcode_protocol.v4.sessions_index_legacy_remote_claim_failed",
            module: "bootstrap.zcode_protocol",
            workspaceId,
          });
        }
      ...
      context.logger?.warn("sessions-index stored summaries failed", {
        error: error instanceof Error ? error.message : String(error),
        event: "qcode_protocol.v4.sessions_index_stored_failed",
        module: "bootstrap.zcode_protocol",
      });
      return [];
    }
  ...
  return new ConversationV4Gateway({
    ...
    resumePersistedSession: async (
      ...
      if (!persisted) {
        context.logger?.warn("ZCode Protocol v4 cold resume has no persisted session", {
          ...
          event: "qcode_protocol.v4.resume_persisted_missing",
          module: "bootstrap.zcode_protocol",
          sessionId,
        });
    ...
    loadPersistedEvents: async (sessionId, persistedMessages) => {
      ...
      if (!record) {
        ...
        context.logger?.warn("ZCode Protocol v4 hydrate has no active runtime", {
          activeSessionCount: context.sessions.size,
          event: "qcode_protocol.v4.hydrate_runtime_missing",
          module: "bootstrap.zcode_protocol",
                      ...
                      sessionEntries: (input) =>
                        store.sessionEntries!(input).catch((error) => {
                          context.logger?.warn("v4 hydrate session entries read failed", {
                            error: error instanceof Error ? error.message : String(error),
                            event: "qcode_protocol.v4.hydrate_session_entries_failed",
                            module: "bootstrap.zcode_protocol",
                          });
                          return [];
                        }),
      ...
      const fileChangeSummariesByMessageId = await buildColdFileChangeSummaries({
        ...
        onArtifactError: (messageId, error) =>
          context.logger?.warn("v4 cold file change artifact read failed", {
            error: error instanceof Error ? error.message : String(error),
            event: "qcode_protocol.v4.hydrate_file_changes_failed",
            messageId,
            module: "bootstrap.zcode_protocol",
            sessionId,
          }),
      ...
      for (const diagnostic of merged.diagnostics) {
        const fields = {
          ...diagnostic,
          event: "qcode_protocol.v4.hydrate_three_source_merge",
          module: "bootstrap.zcode_protocol",
          sessionId,
        };
        if (
    ...
    onError: (scope, error, errorContext) =>
      context.logger?.warn("ZCode Protocol v4 gateway error", {
        ...errorContext,
        error: error instanceof Error ? error.message : String(error),
        event: "qcode_protocol.v4.gateway_error",
        module: "bootstrap.zcode_protocol",
        scope,
      }),
  });
