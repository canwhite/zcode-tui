...
| **R-030** 通道清单 | 四通道表 | 四通道各自有对应控制手段；① 有运行时证据、② 有结构闭合、③ 有源码断言、④ 由「侧问自身零 logger 调用」+ 适配层既有脱敏覆盖。 |
...
3. **R-003 的模型遵从度**：10 个诱导型无据问题要求 10/10 拒答，以及同量级的「确实在上下文内」问题要求正常作答。属于模型行为，只能在真实会话里压测。
...
| P-3 | 配一个 provider，跑完 §3 的三项 + 计划里的手工清单（尤其「长命令执行中输入侧问」与逃生演练，以及 PM-6 的「提交后输入框清空」） | HITL |
