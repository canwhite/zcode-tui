...
export function executionProjection(
  ...
  return {
    "qcode.execution.session_id": options.includeSession
      ? safeId(correlation.sessionId)
      : undefined,
    "qcode.execution.parent_session_id": options.includeSession
      ? safeId(correlation.parentSessionId)
      : undefined,
    "qcode.execution.turn_id": safeId(correlation.turnId),
    "qcode.execution.parent_turn_id": options.includeSession
      ? safeId(correlation.parentTurnId)
      : undefined,
    "qcode.execution.query_id": options.includeQuery ? safeId(correlation.queryId) : undefined,
    "qcode.execution.actor_kind": options.includeActor
      ? safeEnum(correlation.actorKind)
      : undefined,
    "qcode.execution.agent_name": options.includeAgent
      ? safeIdentifier(correlation.agentName)
      : undefined,
    "qcode.execution.identity_state": options.includeIdentity
      ? safeEnum(correlation.identityState)
      : undefined,
    "qcode.execution.user_subject_id": options.includeIdentity
      ? safeId(correlation.userSubjectId)
      : undefined,
    "qcode.execution.launch_surface": safeEnum(correlation.launchSurface),
  };
}
