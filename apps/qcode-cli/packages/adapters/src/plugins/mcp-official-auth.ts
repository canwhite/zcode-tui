/*
 * Plugin `.mcp.json` 里 `auth: { type: "qcode_official" }` 的严格解析与 provenance 生成。
 *
...
export function parseZCodeOfficialAuth(
  ...
  // 精确值匹配，区分大小写；zcode-official、qcode_official_auth 等别名一律拒绝。
  if (value.type !== "qcode_official") {
  ...
  return { type: "qcode_official", provider: "jwt_token" };
}
