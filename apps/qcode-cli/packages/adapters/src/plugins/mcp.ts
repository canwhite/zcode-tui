...
import { findOfficialMcpReservedHeaders } from "@qcode/shared";
import type {
  McpOAuthConfig,
  McpServerConfig,
...
function resolveMcpServerConfig(
  ...
  const source: McpServerRuntimeSource = {
    kind: context.loaded.marketplace === QCODE_OFFICIAL_PLUGIN_MARKETPLACE ? "builtin" : "plugin",
  };

  // qcode_official 允许 http 与 stdio，sse 出现即禁用该 MCP，不静默忽略——静默会让配置作者以为鉴权已生效。
  //
  ...
  if (type === "stdio") {
    ...
    const env = resolveStringRecord(
      {
        ...
        QCODE_PLUGIN_DATA: context.dataPath,
        QCODE_PLUGIN_ROOT: context.loaded.rootPath,
        QCODE_PROJECT_DIR: context.workingDirectory,
    ...
    env[QCODE_PLUGIN_ID_ENV_KEY] = context.loaded.id;
    return {
      type: "stdio",
