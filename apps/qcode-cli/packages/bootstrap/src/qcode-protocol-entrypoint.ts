...
} from "./qcode-protocol/storage-startup.js";
...
import { installZCodeProtocolAiSdkWarningLogger } from "./qcode-protocol/ai-sdk-warning-logger.js";
import {
  createOfficialMcpAuthHeadersPort,
  type OfficialMcpAuthRequestContext,
} from "./qcode-protocol/official-mcp-auth-port.js";
import {
...
import { ZCodeProtocolAgentServer } from "./qcode-protocol/server.js";
import { ZCodeProtocolNdjsonConnection } from "./qcode-protocol/transport.js";
import { cleanupProtocolRuntime } from "./qcode-protocol/runtime-cleanup.js";
import { startProtocolResourceSampler } from "./qcode-protocol/resource-sampler.js";
import { acquireProtocolStartupResource } from "./qcode-protocol/startup-resource.js";
...
export async function runZCodeProtocolAgent(
  ...
  startupTimer.start("ZCode Protocol agent startup started", {
    ...
    event: "qcode_protocol.startup.started",
    stage: "start",
  });
  ...
  try {
        ...
        openProtocolStartupStorage({
          ...
          onProgress: (progress) =>
            logger.info("SQLite startup state", {
              event: "qcode_protocol.startup.storage_state",
              ...progress,
            }),
        }),
    });
    ...
    startupTimer.complete("ZCode Protocol agent startup completed", {
      event: "qcode_protocol.startup.completed",
      stage: "total",
    });
    ...
    startupTimer.fail("ZCode Protocol agent startup failed", error, {
      event: "qcode_protocol.startup.failed",
      stage: "total",
    });
    throw error;
  } finally {
