...
Run the CLI you want to inspect with the environment values shown in the Network panel, usually `QCODE_HTTP_PROXY` and `QCODE_AGENT_CA_CERT`. The agent derives standard proxy and CA variables only inside controlled subprocess boundaries. Disable the proxy with `QCODE_DEBUG_NETWORK_CAPTURE=0` when you only want log/DB inspection.
