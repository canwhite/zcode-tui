...
export const zcodeProcessDiagnosticSchema = z
  .object({
    ...
    name: z.string().min(1).max(QCODE_PROCESS_DIAGNOSTIC_NAME_MAX_CHARS),
    message: z.string().max(QCODE_PROCESS_DIAGNOSTIC_MESSAGE_MAX_CHARS),
    stack: z.string().max(QCODE_PROCESS_DIAGNOSTIC_STACK_MAX_CHARS).optional(),
...
export function parseZCodeProcessDiagnostic(line: string): ZCodeProcessDiagnostic | undefined {
  if (
    !line.startsWith(QCODE_PROCESS_DIAGNOSTIC_PREFIX) ||
    line.length > QCODE_PROCESS_DIAGNOSTIC_MAX_LINE_CHARS
  ) {
    return undefined;
  }
  try {
    const result = zcodeProcessDiagnosticSchema.safeParse(
      JSON.parse(line.slice(QCODE_PROCESS_DIAGNOSTIC_PREFIX.length)),
    );
